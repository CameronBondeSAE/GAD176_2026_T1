...
namespace Sabre.AI
{
public class FindWeaponState : AntAIState
{
    ...
    public override void Enter()
    {
        ...
        BaseCollectable weapon = FindWeapon();

        pathfind.targetTransform = weapon.gameObject.transform;
    ...
    private BaseCollectable FindWeapon()
    {
        foreach(BaseCollectable item in Senses.inventory.nearCollectables)
        {
            if(item.ObjectType == CollectableType.Weapon && item.currentlyHeld == false)
            {
                return item;
            }
        }
